...
export function getDashboardToday(
  ...
  const revenueTodayIdr = doneItems.reduce((sum, q) => sum + (q.priceIdr ?? 0), 0);
  ...
  const barberStats: BarberStats[] = barbers.map((barber) => {
    ...
    return {
      ...
      revenueIdr: barberDoneItems.reduce((sum, q) => sum + (q.priceIdr ?? 0), 0),
    };
  });
...
export function confirmBooking(
  ...
  const startDate = new Date(booking.startAt ?? 0);
  ...
  const msg = [
    ...
    `• Tanggal  : ${dateStr}`,
    `• Jam      : ${timeStr} WIB`,
    `• Harga    : Rp ${(populated.service.priceIdr ?? populated.service.priceIDR ?? 0).toLocaleString('id-ID')}`,
    ``,
  ...
  const rawPhone = (populated.customer.phone ?? populated.customer.phoneRaw ?? '').replace(/\D/g, '');
...
export function getWeeklyStats(tenantId: string): {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    days.push({
      date: dateStr,
      label,
      revenueIdr: dayItems.reduce((s, q) => s + (q.priceIdr ?? 0), 0),
  ...
  return {
    ...
    topBarbers,
    topServices,
    totalRevenue: days.reduce((s, d) => s + (d.revenueIdr ?? 0), 0),
    totalCustomers: days.reduce((s, d) => s + (d.customerCount ?? 0), 0),
  };
}
